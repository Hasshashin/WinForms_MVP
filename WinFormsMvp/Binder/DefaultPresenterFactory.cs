...
namespace WinFormsMvp.Binder
{
    ...
    public class DefaultPresenterFactory : IPresenterFactory
    {
        ...
        public IPresenter Create(Type presenterType, Type viewType, IView viewInstance)
        {
            ...
            try
            {
                //ContainerBootstrapper.Container.Register(Component.For(viewInstance.GetType()))
                var concrete = ContainerBootstrapper.Resolve(presenterType);
                //return (IPresenter)buildMethod.Invoke(null, new[] { viewInstance });
                return (IPresenter)concrete;
            }
            catch (Exception ex)
            {
